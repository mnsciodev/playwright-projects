        ...
        for (const record of group.records) {
            ...
            try {
                ...
                for (const doc of labResults.filter(d => d.visit_date === targetDate)) {
                    ...
                    // await uploadToSFTP(
                    //     filePath,
                    //     `ClaimDocuments/CCMG/Medical records/${groupAccount}/${moment(groupDate).format("YYYYMMDD")}/${path.basename(filePath)}`
                    // );
...
async function runAutomation() {
    ...
    const allRecords = await db.collection("aiclaimmasters").aggregate([
        { $match: { NeedToCheck: "Yes" ,PracticeId : new ObjectId("641be9089ab39a2f9fff3239")} },
        {
            $group: {
