...
(async () => {
    ...
    try {
        ...
        for (const record of searchRecords) {
            ...
            try {
                const errorAlert = frame.locator('div.alert.alert-danger');

                await errorAlert.waitFor({ state: 'visible', timeout: 8000 });

                const alertText = await errorAlert.innerText();

                if (alertText.includes('Data Search Unsuccessful') || alertText.toLowerCase().includes('not found')) {
                    console.warn(
                    ...
                    console.warn(alertText.trim());
                    continue; // move to next input record
                }

            } catch (err) {
                // Alert did not appear → this is OK
                // Means search probably returned results
            }

            await frame.waitForSelector('#claimsTable tbody tr', { timeout: 60000 });
