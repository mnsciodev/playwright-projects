...
(async () => {
  try {
    ...
    // var GetUsersList = []
    // for (users in GetProvider) {
    //   GetUsersList.push(GetProvider.ProviderName)
    // }
    // if (!users.length) {
    ...
    //   renderingProviderName: { $in: GetUsersList },
    //   process: "inactive",
    // });
    // if (claims.length == 0) {
    ...
    var users = await Claim.aggregate(
      [
        {
          $match: {
            createdAt: {
