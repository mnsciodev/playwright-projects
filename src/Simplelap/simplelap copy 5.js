                ...
                try {
                    ...
                    for (const row of rows) {
                        ...
                        if (collectionDate.includes(patient.dos)) {
                            ...
                            const resultPdfPath = path.join(baseDir, `${patient.ResidentID}_Results_${cleanDate}.pdf`);
                            const orderPdfPath = path.join(baseDir, `${patient.ResidentID}_Order_${cleanDate}.pdf`);
                            const mergedPdfPath = path.join(baseDir, `${patient.ResidentID}_Merged_${cleanDate}.pdf`);
                            ...
                            //     const sourceDetailsBar = popupOrder.locator('#sourceDetailsBar');
                            //     if (await sourceDetailsBar.isVisible()) await sourceDetailsBar.click();
                            //     const auditDetailsBar = popupOrder.locator('#auditDetailsBar');
                            //     if (await auditDetailsBar.isVisible()) await auditDetailsBar.click();
                            ...
                            // } else {
                            //     logPatient(patient, `No Order Found`);
                            // }
                            const actionsMenuOrder = await row.$('a.pccActionMenu');
                            await actionsMenuOrder.click();
                            ...
                            if (viewOrder) {
                                ...
                                // Expand collapsible sections if visible
                                for (const id of ['#scheduleDetailsBar', '#sourceDetailsBar', '#auditDetailsBar']) {
                                    const section = popupOrder.locator(id);
                                ...
                                // 🔍 Zoom out and remove scrollbars completely
                                await popupOrder.evaluate(() => {
                                    document.body.style.zoom = '0.55';            // zoom out
                                    document.documentElement.style.overflow = 'hidden'; // hide scrollbar
                                    document.body.style.overflow = 'hidden';
                                });

                                // ✅ Resize viewport so Playwright renders full page in one frame (no scroll cut)
                                ...
                                await popupOrder.pdf({
                                    ...
                                    scale: 1,              // Keep PDF scale normal since you zoomed the page
